...
import { 
  Package, ArrowDownToLine, Send, Users, Mic, Route, MessageCircle, Cpu, Smartphone, Bot, Globe, Database, X, Loader2, LayoutDashboard, Search
} from 'lucide-react';
...
const callGeminiAPI = async (prompt: string, systemInstruction = "Eres el consultor logístico y asistente experto AI de WarehouseFlow. Tus respuestas deben ser sumamente profesionales, concisas y directas.") => {
  const apiKey = getGeminiApiKey();
  if (!apiKey) {
    await new Promise(res => setTimeout(res, 800));
    if (prompt.toLowerCase().includes('json')) {
      if (prompt.includes('inventory')) {
        return JSON.stringify({ sku: `SKU-${Math.floor(100 + Math.random() * 900)}`, name: 'Producto IA Generado', category: 'Almacenaje', stock: 120, minStock: 20, location: 'A-05-01', price: 45.00 });
      }
      if (prompt.includes('crm') || prompt.includes('cliente')) {
        return JSON.stringify({ code: `CUST0${Math.floor(10 + Math.random() * 90)}`, name: 'Empresa Logística IA', type: 'Cliente', email: 'ia@empresalogistica.com', phone: '+34 600 000 000', status: 'Activo' });
      }
      if (prompt.includes('order') || prompt.includes('orden')) {
        return JSON.stringify({ orderNumber: `PED-2026-${Math.floor(100 + Math.random() * 900)}`, customerName: 'Mercadona S.A.', status: 'Pendiente', priority: 'normal', totalItems: 6, totalValue: 850.00 });
      }
        ...
        <div className="fixed inset-0 bg-[#050811]/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0b0f19] border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsApiKeyModalOpen(false)} className="text-slate-400 hover:text-white transition">
                <X size={20} />
              </button>
            </div>
            <p className="text-xs text-slate-400 mb-4">
              Introduce tu clave API de Gemini. Se guardará de forma local en tu navegador para habilitar la generación de respuestas reales y autocompletado en el SGA.
            </p>
            <input 
              type="password" 
            ...
            <div className="flex justify-end space-x-3 text-sm">
              <button 
                onClick={() => { setTempApiKey(''); localStorage.removeItem('gemini_api_key'); setHasApiKey(false); setIsApiKeyModalOpen(false); }}
