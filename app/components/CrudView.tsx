...
const getStatusColor = (status: string) => {
  ...
  if (redStatuses.includes(status))
    return "bg-rose-500/20 text-rose-400 border border-rose-550/30";
...
export default function CrudView({
  ...
  const handleBatchDelete = () => {
    if (
      onBatchDelete &&
      confirm(`¿Eliminar ${selectedIds.length} registros seleccionados?`)
    ) {
  ...
  const hasActiveFilters =
    searchQuery.trim() || Object.values(activeFilters).some((v) => v);
