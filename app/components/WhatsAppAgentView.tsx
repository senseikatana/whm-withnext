...
const callGeminiLocal = async (prompt: string) => {
  ...
  const payload = {
    ...
    systemInstruction: { parts: [{ text: "Eres el asistente y coordinador logístico de WhatsApp de WarehouseFlow." }] }
  };
  try {
...
export default function WhatsAppAgentView({ chats, setDbState }: WhatsAppAgentViewProps) {
  ...
  const handleGenerateAIResponse = async () => {
    ...
    const res = await callGeminiLocal(`Responde de manera formal y directa a este mensaje de logística de WhatsApp: "${activeChat.message}"`);
    setReplyText(res);
